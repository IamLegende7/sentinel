...
bool HitboxQuadtree::add_entry(Node* node, Hitbox hitbox) {
    ...
    if (node->children[0] != nullptr) {
        ...
        if (hitbox_length > node_capacity) {
            if (DEBUG_ALL_DEBUG_LOGS) LOGGER.log(LogLevel::DEBUG, "[hitboxes.cpp:add_entry] Subdividing node at %d %d and %d %d", node->min.x, node->min.y, node->max.x, node->max.y);
            subdivide(node);
...
void HitboxQuadtree::subdivide(Node* node) {
    if (DEBUG_ANNOYING_LOGS) LOGGER.log(LogLevel::DEBUG, "Splitting node: %d %d to %d %d", node->min.x, node->min.y, node->max.x, node->max.y);
    ...
    if (DEBUG_ANNOYING_LOGS) LOGGER.log(LogLevel::DEBUG, "Mid of node: %d %d", mid.x, mid.y);

    // Create children //
    if (DEBUG_ANNOYING_LOGS) LOGGER.log(LogLevel::DEBUG, "New node: %d %d to %d %d", node->min.x, node->min.y, mid.x, mid.y);
    if (DEBUG_ANNOYING_LOGS) LOGGER.log(LogLevel::DEBUG, "New node: %d %d to %d %d", mid.x, node->min.y, node->max.x, mid.y);
    if (DEBUG_ANNOYING_LOGS) LOGGER.log(LogLevel::DEBUG, "New node: %d %d to %d %d", node->min.x, mid.y, mid.x, node->max.y);
    if (DEBUG_ANNOYING_LOGS) LOGGER.log(LogLevel::DEBUG, "New node: %d %d to %d %d", mid.x, mid.y, node->max.x, node->max.y);
